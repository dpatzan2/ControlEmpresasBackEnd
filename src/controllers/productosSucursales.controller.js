...
function ObtenerProductosSucursales(req, res) {
    ...
    if(req.user.rol == 'Empresa'){
        ProductosSucursales.find({idEmpresa: req.user.sub,idSucursal:idSuc}, (err, productoEncontrado) => {
            if (err ||!productoEncontrado) return res.status(404).send( { mensaje: 'La sucursal no posee productos' });
    
            return res.status(200).send({ producto: productoEncontrado });
        });
    }else {
            return res.status(200).send({ mensaje: 'Solamente la empresa puede ver sus productos de sucursal' });
    }
...
function agregarProductoSucursales(req, res){
    ...
    if(req.user.rol != 'Empresa') return res.status(500).send({ mensaje: 'Únicamente las empresas pueden acceder a esta gestión'})

    if(parametros.vendido) return res.status(500).send({ mensaje: "EL campo vendido no se puede agregar" });
    if(parametros.StockSucursal<=0) return res.status(500).send({ mensaje: "EL stock debe ser mayor a 0. Verifique los datos"})
    ...
    if(parametros.NombreProductoSucursal && parametros.StockSucursal && parametros.NombreSucursal
        && parametros.NombreProductoSucursal!="" && parametros.StockSucursal!="" && parametros.NombreSucursal!=""){
                ...
                Sucursales.findOne({ nombreSucursal: parametros.NombreSucursal,idEmpresa:req.user.sub}, (err, sucursalEncontrada)=>{
                     if(err) return res.status(400).send({ mensaje: 'Error. Esta Sucursal no existe. Verifique el nombre'});
                    if(!sucursalEncontrada) return res.status(400).send ({ mensaje: 'Esta Sucursal no existe en la empresa. Verifique el nombre'})
                    ...
                    ProductosSucursales.findOne({NombreProductoSucursal:parametros.NombreProductoSucursal,idSucursal:sucursalEncontrada._id},(err, productoEncontradoSucursal)=>{
                        if(err) return res.status(404).send({mensaje:'Error.Verifique los datos'})
                        ...
                        if(productoEncontradoSucursal==null){//EL PRODUCTO NO EXISTE EN SUCURSALES Y SE AGREGA NORMAL
                            ...
                            ProductosEmpresa.findOne({ NombreProducto: parametros.NombreProductoSucursal,idEmpresa:req.user.sub}, (err, productoEmpresaStock)=>{
                                if(err) return res.status(400).send({ mensaje: 'Error. Esta Sucursal no existe. Verifique el nombre'});
                               if(!productoEmpresaStock) return res.status(400).send ({ mensaje: 'Este producto no existe en la empresa. Verifique los datos'})
           
                               if(parametros.StockSucursal<=0) return res.status(500).send({ mensaje: 'La cantidad no puede ser menor o igual a cero'});

                               if(productoEmpresaStock.Stock==0) return res.status(500).send({ mensaje: 'Producto agotado. No es posible agregar el producto a sucursales.'})

                               if(parametros.StockSucursal>productoEmpresaStock.Stock){//VERIFICAR STOCK
      
                                   return res.status(500).send({ mensaje: 'La cantidad del producto es es mayor al stock. El Stock actual del producto es: '+productoEmpresaStock.Stock});
                               }
        ...
        return res.status(500).send({mensaje: 'Ingrese todos los datos necesarios'});
...
function EliminarProductoSucursales(req, res){
    ...
    if(req.user.rol == 'Empresa'){
        ...
        return res.status(200).send({ mensaje: 'Solamente la empresa puede eliminar sus productos de sucursal' });
    }
}
